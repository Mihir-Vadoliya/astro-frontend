...
class BinomClickApi{
        ...
        **/
        ...
        function __construct($CampaignKey){
            $CampaignLink = "https://dealstrack.online/click.php?key={$CampaignKey}";
        ...
        public function setLPClick(){
            $URL=$this->getLPClickURL();
            Log::debug('Fire getLPClick URL = ' . $URL);
            $ch = curl_init();
            ...
            $result = curl_exec( $ch );
            Log::debug('Fire getLPClick result = ' . serialize($result));
            curl_close( $ch );
            return true;
        }
