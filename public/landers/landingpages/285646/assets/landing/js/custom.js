...
$(document).ready(function () {
	...
	// step 10 show
	$(".step_9_next, .step_9_next_arrow").on('click',function () {
		...
		if(email==''){
			...
			if(mail != true){
				...
				$('.step_9_error_2').removeClass('d-none');
			}else{
				$('.step_9_error').addClass('d-none');
				$('.step_9_error_2').addClass('d-none');
               $("form").submit();
			}
		}
	});
	...
	$(".step_11_prev").on('click',function () {
		back('.step_9', '.step_11');
	});
});
...
$(document).on('keypress',function (e) {
	if (e.keyCode == 13) {
		for(var i=2; i<=9; i++){
			...
			if(!$(stepClass).hasClass('d-none')){
				$('.step_'+i+'_next').trigger('click');
				break;
			}
		}
	}
